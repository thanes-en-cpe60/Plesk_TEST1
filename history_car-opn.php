...
include('server.php');
    //---------job_id มาจากhistory_car
    if (isset($_GET["To_his_opn"])) {
        $car_id = $_GET['To_his_opn'];
        $sql = mysqli_query($conn, "SELECT job_id FROM car WHERE car_id ='$car_id'");
        $row = $sql->fetch_assoc();
    }  
?>
...
<!-- -------------------------------- Main --------------------------------- -->
            ...
            <div class="layout">
              <h3> <a href="history_car.php">ประวัติรถ</a> > <?php echo $car_id ?> </h3>
